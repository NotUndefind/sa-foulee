...
export default function SessionForm({ session, templateSource, onSuccess, onCancel }: Props) {
  ...
  // Sync exercises quand source change
  useEffect(() => {
  ...
  const inputCls = 'w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/20'
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="flex items-center gap-0">
        {STEPS.map((label, idx) => (
          <div key={idx} className="flex items-center flex-1">
            ...
            <span className={`ml-1.5 text-xs font-medium ${step === idx ? 'text-zinc-800' : 'text-zinc-400'}`}>
              {label}
            </span>
            {idx < STEPS.length - 1 && (
              <div className={`mx-3 flex-1 h-0.5 ${idx < step ? 'bg-accent' : 'bg-zinc-200'}`} />
            )}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Distance (km) <span className="text-zinc-400">optionnel</span></label>
              <input
                type="number"
                step="0.1"
                min="0"
            ...
            <div>
              <label className={labelCls}>Durée (min) <span className="text-zinc-400">optionnel</span></label>
              <input
                type="number"
                min="1"
      ...
      {step === 1 && (
        <div className="space-y-3">
          <p className="text-sm text-zinc-500">
            Ajoutez les exercices structurés de la session. Laissez vide si la session est une course libre.
          </p>
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer rounded-xl border border-zinc-200 p-3 hover:bg-zinc-50">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Réutilisable pour créer de futures sessions rapidement.</p>
            </div>
          </label>
      ...
      <div className="flex justify-between gap-3 pt-2 border-t border-zinc-100">
        <button
          type="button"
        ...
        {step < STEPS.length - 1 ? (
          ...
          >
            {isSubmitting ? 'Enregistrement…' : isEdit ? 'Modifier' : isTemplate ? 'Sauvegarder le template' : 'Publier la session'}
          </button>
        )}
      </div>
    </form>
