...
const ROLE_LABELS: Record<string, string> = {
  admin: 'Admin', founder: 'Fondateur', coach: 'Entraîneur', bureau: 'Bureau', member: 'Membre',
}
...
const DOC_STATUS_LABELS: Record<string, string> = {
  valid: 'Valide', pending: 'En attente', expired: 'Expiré',
}

const REQUIRED_TYPES = ['license', 'medical_certificate', 'registration']
...
function ProgressRing({ pct, size = 28 }: { pct: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} className="-rotate-90" viewBox={`0 0 ${size} ${size}`}>
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="rgba(192,48,46,0.1)" strokeWidth="3" />
      <circle
    ...
    <tr style={{ borderBottom: '1px solid rgba(192,48,46,0.05)' }}>
      {[1,2,3,4,5].map((i) => (
        <td key={i} className="px-4 py-3">
          <div className="h-4 rounded animate-pulse" style={{ width: `${60 + i * 10}%`, background: 'rgba(192,48,46,0.06)' }} />
        </td>
      ))}
    </tr>
  )
...
function SkeletonPendingRow() {
  return (
    <div className="flex items-center gap-4 rounded-xl bg-white px-5 py-4 animate-pulse" style={{ border: '1px solid rgba(192,48,46,0.07)' }}>
      <div className="h-10 w-10 rounded-full shrink-0" style={{ background: 'rgba(192,48,46,0.06)' }} />
...
function SlideOverPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <div
        className="relative ml-auto z-10 flex h-full w-full max-w-md flex-col bg-white shadow-2xl"
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-start justify-between px-6 py-5" style={{ borderBottom: '1px solid rgba(192,48,46,0.07)' }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold leading-tight" style={{ color: '#C0302E' }}>
                {user.first_name} {user.last_name}
              </h2>
              <p className="text-xs mt-0.5" style={{ color: '#7F7F7F' }}>{user.email}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4" style={{ borderBottom: '1px solid rgba(192,48,46,0.07)' }}>
          <div className="flex items-center justify-between mb-2.5">
            <span className="text-xs font-medium uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Complétion du dossier</span>
            <span className="text-sm font-bold tabular-nums" style={{ color: completion === 100 ? '#D42F2D' : '#f97316' }}>
              {completion}%
            </span>
          </div>
          <div className="h-1.5 w-full rounded-full overflow-hidden" style={{ background: 'rgba(192,48,46,0.08)' }}>
            <div
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          <p className="mb-3 text-xs font-medium uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Documents déposés</p>
          {loading ? (
            <div className="space-y-2.5">
              {[1,2,3].map((i) => (
                <div key={i} className="h-16 rounded-xl animate-pulse" style={{ background: 'rgba(192,48,46,0.05)' }} />
              ))}
            </div>
          ) : docs.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full" style={{ background: 'rgba(192,48,46,0.06)' }}>
                <svg viewBox="0 0 24 24" className="h-5 w-5" style={{ color: '#7F7F7F' }} fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2z" />
                </svg>
              </div>
              <p className="text-sm font-medium" style={{ color: '#C0302E' }}>Aucun document</p>
              <p className="mt-1 text-xs" style={{ color: '#7F7F7F' }}>Ce membre n'a encore uploadé aucun fichier.</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium truncate leading-snug" style={{ color: '#C0302E' }}>{doc.filename}</p>
                      <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                        ...
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${DOC_STATUS_COLORS[doc.status] ?? ''}`}>
                  ...
                  <div className="mt-2.5 flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center rounded-lg border border-zinc-200 p-1.5 text-zinc-400 hover:border-red-200 hover:bg-red-50 hover:text-red-500 transition-colors"
                      title="Supprimer"
                    >
                      <svg viewBox="0 0 14 14" className="h-3 w-3" fill="none" stroke="currentColor" strokeWidth="2">
...
export default function AdminUsersPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-6" style={{ minHeight: '100vh' }}>

      {/* ── Page header ── */}
      <div>
        <h1 className="text-2xl font-bold tracking-tight" style={{ color: '#C0302E' }}>Administration</h1>
        <p className="mt-1 text-sm" style={{ color: '#7F7F7F' }}>Gestion des membres et des dossiers de l'association</p>
      </div>

      {/* ── Tabs ── */}
      <div className="flex items-center gap-1 rounded-xl p-1 w-fit" style={{ border: '1px solid rgba(192,48,46,0.1)', background: 'rgba(192,48,46,0.04)' }}>
        {([
          ...
          <button
            ...
            style={activeTab === key
              ? { background: 'white', color: '#C0302E', boxShadow: '0 1px 4px rgba(192,48,46,0.1)' }
              : { color: '#7F7F7F' }
            }
          >
            {label}
      ...
      {activeTab === 'pending' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {/* En attente */}
            <div className="flex items-center gap-4 rounded-xl bg-white px-5 py-4" style={{ border: '1px solid rgba(192,48,46,0.1)' }}>
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: 'rgba(251,57,54,0.08)' }}>
                <svg viewBox="0 0 20 20" className="h-5 w-5" style={{ color: '#FB3936' }} fill="none" stroke="currentColor" strokeWidth="1.5">
                  <circle cx="10" cy="10" r="8" />
                  <path d="M10 6v4l2.5 2.5" strokeLinecap="round" />
                </svg>
              </div>
              <div>
                <p className="text-xs leading-none mb-1" style={{ color: '#7F7F7F' }}>En attente</p>
                <p className="text-2xl font-bold tabular-nums" style={{ color: '#C0302E' }}>{pendingLoading ? '—' : pendingCount}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 rounded-xl bg-white px-5 py-4" style={{ border: '1px solid rgba(192,48,46,0.1)' }}>
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: 'rgba(169,50,38,0.08)' }}>
                <svg viewBox="0 0 20 20" className="h-5 w-5" style={{ color: '#D42F2D' }} fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M4 10l4.5 4.5L16 6" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                <p className="text-xs leading-none mb-1" style={{ color: '#7F7F7F' }}>Validés ce mois</p>
                <p className="text-2xl font-bold tabular-nums" style={{ color: '#C0302E' }}>—</p>
              </div>
            </div>

            {/* Dossiers complets */}
            <div className="flex items-center gap-4 rounded-xl bg-white px-5 py-4" style={{ border: '1px solid rgba(192,48,46,0.1)' }}>
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ background: 'rgba(192,48,46,0.06)' }}>
                <svg viewBox="0 0 20 20" className="h-5 w-5" style={{ color: '#D42F2D' }} fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M3 7l4 2 3-4 3 6 2-2 3 3" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              <div>
                <p className="text-xs leading-none mb-1" style={{ color: '#7F7F7F' }}>Dossiers complets</p>
          ...
          {/* Document list */}
          <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid rgba(192,48,46,0.1)', background: 'rgba(192,48,46,0.02)' }}>
            {pendingLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-base font-semibold" style={{ color: '#C0302E' }}>Tous les dossiers sont à jour</p>
                <p className="mt-1.5 text-sm" style={{ color: '#7F7F7F' }}>Aucun document en attente de validation.</p>
              </div>
            ) : (
              <div style={{ divide: 'y' }}>
                {pendingDocs.map((doc) => (
                  ...
                  >
                    ...
                    {/* Member info */}
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold leading-snug" style={{ color: '#C0302E' }}>
                        {doc.user.first_name} {doc.user.last_name}
                      </p>
                      <p className="text-xs truncate" style={{ color: '#7F7F7F' }}>{doc.user.email}</p>
                    </div>

                    {/* Doc type badge */}
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${DOC_TYPE_BADGE[doc.type] ?? DOC_TYPE_BADGE.other}`}>
                      {DOC_TYPE_LABELS[doc.type] ?? doc.type}
                    </span>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              <svg viewBox="0 0 16 16" className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4" style={{ color: '#7F7F7F' }} fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* Table */}
          <div className="overflow-hidden rounded-2xl bg-white" style={{ border: '1px solid rgba(192,48,46,0.1)' }}>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(192,48,46,0.08)', background: '#F8F8F8' }}>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Membre</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Rôle</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Dossier</th>
                    <th className="hidden px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide sm:table-cell" style={{ color: '#7F7F7F' }}>Inscrit le</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wide" style={{ color: '#7F7F7F' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {usersLoading ? (
                    ...
                    users.map((user) => (
                      <tr
                        ...
                        style={{ borderBottom: '1px solid rgba(192,48,46,0.05)' }}
                        onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(192,48,46,0.02)')}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="font-medium leading-snug" style={{ color: '#C0302E' }}>
                                {user.first_name} {user.last_name}
                              </p>
                              <p className="text-xs truncate max-w-[180px]" style={{ color: '#7F7F7F' }}>{user.email}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        {/* Date cell */}
                        <td className="hidden px-4 py-3 text-xs sm:table-cell" style={{ color: '#7F7F7F' }}>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              ...
                              className="flex items-center justify-center rounded-lg border border-zinc-200 p-1.5 text-zinc-400 hover:border-red-200 hover:bg-red-50 hover:text-red-500 transition-colors disabled:opacity-50"
                              title="Supprimer ce membre"
                            >
                              <svg viewBox="0 0 14 14" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="1.8">
                                <path d="M2 3.5h10M5 3.5V2.5a.5.5 0 01.5-.5h3a.5.5 0 01.5.5v1M4.5 3.5l.5 8h5l.5-8" strokeLinecap="round" />
                              </svg>
                            </button>
                          </div>
                        </td>
            ...
            {!usersLoading && meta.last_page > 1 && (
              <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: '1px solid rgba(192,48,46,0.06)' }}>
                <p className="text-xs" style={{ color: '#7F7F7F' }}>
                  Page <span className="font-medium" style={{ color: '#C0302E' }}>{meta.current_page}</span> sur{' '}
                  <span className="font-medium" style={{ color: '#C0302E' }}>{meta.last_page}</span>
