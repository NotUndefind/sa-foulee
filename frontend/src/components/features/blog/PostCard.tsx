...
export default function PostCard({ post, onUpdate, onDelete, onEdit, canPin }: Props) {
  ...
  return (
    <article
      ...
      style={{ boxShadow: '0 2px 10px rgba(30,58,20,0.07)', border: '1px solid rgba(30,58,20,0.08)' }}
    >
      {/* Image */}
      {post.image && (
        <div className="h-48 w-full overflow-hidden" style={{ background: '#F4EFE6' }}>
      ...
      <div className="p-6">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          {post.is_pinned && (
            <span
              ...
              style={{ background: 'rgba(208,89,24,0.1)', color: '#D05918', border: '1px solid rgba(208,89,24,0.2)' }}
            >
              <IconPin /> Épinglé
            </span>
          )}
          {post.published_at && (
            <span className="text-xs" style={{ color: '#7A9E6E' }}>{formatDate(post.published_at)}</span>
          )}
          {post.author && <span className="text-xs" style={{ color: '#7A9E6E' }}>par {post.author.name}</span>}
        </div>

        {/* Titre */}
        <h2 className="text-lg font-bold" style={{ color: '#1E3A14' }}>{post.title}</h2>
        ...
        {/* Actions */}
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t pt-4" style={{ borderColor: 'rgba(30,58,20,0.06)' }}>
          <button
            ...
            {canPin && (
              <button
                ...
                style={post.is_pinned
                  ? { color: '#D05918', background: 'rgba(208,89,24,0.06)' }
                  : { color: '#7A9E6E' }
                }
              >
        ...
        {showComments && (
          <div className="mt-4 space-y-3 border-t pt-4" style={{ borderColor: 'rgba(30,58,20,0.06)' }}>
            {commentsLoading ? (
              <p className="text-center text-xs" style={{ color: '#7A9E6E' }}>Chargement…</p>
            ) : comments.length === 0 ? (
              <p className="text-center text-xs" style={{ color: '#7A9E6E' }}>
              ...
              comments.map((c) => (
                <div key={c.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1 rounded-xl px-3 py-2" style={{ background: '#F9F6F1' }}>
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-xs font-medium" style={{ color: '#1E3A14' }}>
                        {c.user?.name ?? 'Membre'}
                      </span>
                      <span className="text-xs" style={{ color: '#7A9E6E' }}>{formatDate(c.created_at)}</span>
                    </div>
                    <p className="mt-0.5 text-sm" style={{ color: '#3A6B2A' }}>{c.content}</p>
                  </div>
                  {(isAdmin || true) && (
                    <button
                      onClick={() => handleDeleteComment(c.id)}
                      className="mt-1 text-xs transition"
                      style={{ color: 'rgba(30,58,20,0.2)' }}
                    >
                      ✕
                    </button>
                  )}
            ...
            <form onSubmit={handleSubmitComment} className="flex gap-2 pt-1">
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #D05918 0%, #B04A10 100%)' }}
              >
                {submitting ? '…' : 'Envoyer'}
              </button>
            </form>
