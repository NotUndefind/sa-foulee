...
function EquipmentForm({ initial, onSave, onCancel }: FormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const saved = initial ? await updateEquipment(initial.id, form) : await createEquipment(form)
      onSave(saved)
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        >
          {(Object.entries(STATUS_CONFIG) as [EquipmentStatus, { label: string }][]).map(
            ([v, { label }]) => (
              <option key={v} value={v}>
                {label}
              </option>
      ...
      <div>
        <label className="inv-label">
          Notes <span style={{ color: '#7F7F7F', fontWeight: 400 }}>— optionnel</span>
        </label>
        <textarea
      ...
      <div className="flex justify-end gap-3 pt-1">
        <button type="button" onClick={onCancel} className="inv-btn-ghost">
          Annuler
        </button>
        <button type="submit" disabled={saving} className="inv-btn-primary">
...
export default function InventoryPage() {
  ...
  const filtered = items.filter(
    (i) => (!filterCat || i.category === filterCat) && (!filterStat || i.status === filterStat)
  )
    ...
    <>
        ...
        <div className="mx-auto max-w-4xl space-y-5 px-5 py-8">
          ...
          <div>
            ...
            <p className="text-sm" style={{ color: '#7F7F7F' }}>
              Gérez le matériel de l&apos;association
            </p>
          </div>
            ...
            {(
              ...
              <div key={s} className="inv-card" style={{ padding: '16px 20px' }}>
                <p
                  ...
                  style={{ color: STATUS_CONFIG[s].color, lineHeight: 1 }}
                >
                  {counts[s]}
                </p>
            ...
            >
              ...
              <div className="relative">
                ...
                >
                  ...
                  {(Object.entries(CATEGORY_LABELS) as [EquipmentCategory, string][]).map(
                    ([v, l]) => (
                      <option key={v} value={v}>
                        {l}
                      </option>
                    ...
                    {filtered.map((item) => {
                      ...
                      return (
                        <tr
                          ...
                          onClick={() => router.push(`/tableau-de-bord/inventaire/${item.id}`)}
                        >
                          <td className="inv-td font-semibold">{item.name}</td>
                          <td className="inv-td" style={{ color: '#7F7F7F' }}>
                          ...
                          <td className="inv-td">
                            <span
                              style={{
                                fontWeight: 600,
                                color: item.available_count > 0 ? '#059669' : '#D42F2D',
                              }}
                            >
                          ...
                          <td className="inv-td">
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                style={{ color: '#7F7F7F' }}
                                onMouseEnter={(e) =>
                                  (e.currentTarget.style.background = 'rgba(192,48,46,0.08)')
                                }
                              ...
                              {canDelete && (
                                <button
                                  ...
                                  style={{ color: '#7F7F7F' }}
                                  onMouseEnter={(e) =>
                                    (e.currentTarget.style.background = 'rgba(251,57,54,0.08)')
                                  }
                                  onMouseLeave={(e) => (e.currentTarget.style.background = 'none')}
                                >
                                  <IconTrash />
      ...
      {showForm && (
        ...
        >
          ...
          >
            <h3
              style={{ fontSize: '18px', fontWeight: 800, color: '#1A1A1A', marginBottom: '20px' }}
            >
      ...
      {deleting && (
        ...
        >
          ...
          >
            <h3
              style={{ fontSize: '18px', fontWeight: 800, color: '#1A1A1A', marginBottom: '10px' }}
            >
              Supprimer l&apos;équipement ?
            </h3>
            <p style={{ fontSize: '14px', color: '#7F7F7F', marginBottom: '24px' }}>
              <strong style={{ color: '#1A1A1A' }}>{deleting.name}</strong> sera supprimé
              définitivement.
            </p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setDeleting(null)} className="inv-btn-ghost">
                Annuler
              </button>
              <button
                onClick={handleDelete}
                className="rounded-xl px-5 py-2.5 text-sm font-bold text-white"
                style={{ background: '#D42F2D', border: 'none', cursor: 'pointer' }}
              >
                Supprimer
              </button>
            </div>
