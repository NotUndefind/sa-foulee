...
export default function AssignmentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const res = (await assignEquipment(equipmentId, {
        user_id: selected.id,
        ...
        >
          ...
          <div>
            ...
            {search && !selected && (
              ...
              >
                {filtered.length === 0 ? (
                  <p style={{ padding: '12px 14px', fontSize: '13px', color: '#7F7F7F' }}>
                    Aucun résultat.
                  </p>
                ) : (
                  filtered.slice(0, 8).map((m) => (
                    <button
                      ...
                      onMouseEnter={(e) =>
                        (e.currentTarget.style.background = 'rgba(251,57,54,0.04)')
                      }
                      onMouseLeave={(e) => (e.currentTarget.style.background = 'none')}
                    >
                      <p
                        style={{ fontSize: '13.5px', fontWeight: 600, color: '#1A1A1A', margin: 0 }}
                      >
          ...
          <div
            style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: 'auto' }}
          >
            <button
              type="button"
              onClick={onCancel}
