...
export default function NewsletterAdminPage() {
  ...
  useEffect(() => {
    if (!canManageUsers) { router.replace('/tableau-de-bord'); return }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url; a.download = `newsletter-abonnes-${new Date().toISOString().slice(0,10)}.csv`; a.click()
        ...
        <div className="mx-auto max-w-4xl px-5 py-8 space-y-5">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1" style={{ color: '#D42F2D' }}>
              <IconMail />
              <h1 className="text-3xl font-extrabold" style={{ letterSpacing: '-0.02em', color: '#C0302E' }}>Newsletter</h1>
            </div>
            <p className="text-sm" style={{ color: '#7F7F7F' }}>Gérez les abonnés et les campagnes</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="nl-card" style={{ padding: '20px 24px' }}>
              ...
              <p className="text-sm mt-1" style={{ color: '#7F7F7F' }}>abonné{total !== 1 ? 's' : ''}</p>
            </div>
            <div className="nl-card" style={{ padding: '20px 24px' }}>
              <p className="text-4xl font-extrabold" style={{ color: '#FB3936', lineHeight: 1 }}>{campaigns.filter(c => c.sent_at).length}</p>
              <p className="text-sm mt-1" style={{ color: '#7F7F7F' }}>campagne{campaigns.filter(c => c.sent_at).length !== 1 ? 's' : ''} envoyée{campaigns.filter(c => c.sent_at).length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          ...
          <div className="nl-card" style={{ overflow: 'visible' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid rgba(192,48,46,0.07)', flexWrap: 'wrap', gap: '8px' }}>
              <div style={{ display: 'flex', gap: '4px' }}>
                <button className={`nl-tab${tab === 'subscribers' ? ' active' : ''}`} onClick={() => setTab('subscribers')}>
                  Abonnés ({total})
                </button>
                <button className={`nl-tab${tab === 'campaigns' ? ' active' : ''}`} onClick={() => setTab('campaigns')}>
            ...
            {tab === 'subscribers' && (
              <>
                <div style={{ display: 'flex', gap: '12px', padding: '12px 16px', borderBottom: '1px solid rgba(192,48,46,0.05)' }}>
                  <div className="relative flex-1">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#7F7F7F' }}><IconSearch /></span>
                    <input className="nl-input" placeholder="Rechercher par nom ou email…" value={search} onChange={(e) => setSearch(e.target.value)} />
                  </div>
                </div>
                        ...
                        {filtered.map((s) => (
                          <tr key={s.id} className="nl-tr">
                            ...
                            <td className="nl-td" style={{ color: '#7F7F7F', whiteSpace: 'nowrap' }}>
                              {new Date(s.newsletter_subscribed_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {tab === 'campaigns' && (
              <>
                ...
                {!showForm && campaigns.length === 0 && (
                  <div className="flex flex-col items-center gap-2 py-12 text-center">
                    <div style={{ opacity: 0.2, color: '#D42F2D' }}><IconMail /></div>
                    <p className="text-sm" style={{ color: '#7F7F7F' }}>Aucune campagne envoyée pour l&apos;instant.</p>
                ...
                {!showForm && campaigns.length > 0 && (
                  <div className="overflow-x-auto">
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ background: 'rgba(251,57,54,0.02)' }}>
                          <th className="nl-th" style={{ textAlign: 'left' }}>Sujet</th>
                          <th className="nl-th" style={{ textAlign: 'left' }}>Statut</th>
                          <th className="nl-th" style={{ textAlign: 'left' }}>Destinataires</th>
                          <th className="nl-th" style={{ textAlign: 'left' }}>Date d&apos;envoi</th>
                        </tr>
                      </thead>
                      <tbody>
                        {campaigns.map((c) => (
                          <tr key={c.id} className="nl-tr">
                            ...
                            <td className="nl-td" style={{ color: '#7F7F7F' }}>
                              {c.sent_at ? `${c.recipient_count} membre${c.recipient_count !== 1 ? 's' : ''}` : '—'}
                            </td>
                            <td className="nl-td" style={{ color: '#7F7F7F', whiteSpace: 'nowrap' }}>
                              {c.sent_at
