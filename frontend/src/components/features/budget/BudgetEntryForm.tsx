...
const CATEGORIES_DEPENSE = ['matériel', 'déplacement', 'restauration', 'communication', 'assurance', 'location', 'autre']
...
export default function BudgetEntryForm({ entry, defaultType = 'depense', onSaved, onCancel }: Props) {
  const isEdit = !!entry
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: 'white', borderRadius: '20px', padding: '28px 32px', maxWidth: '480px', width: '100%', boxShadow: '0 8px 40px rgba(0,0,0,0.16)', fontFamily: "'Baloo 2', sans-serif", maxHeight: '90vh', overflowY: 'auto' }}>

        <h3 style={{ fontSize: '18px', fontWeight: 800, color: '#1A1A1A', marginBottom: '4px' }}>
          {isEdit ? 'Modifier le mouvement' : typeValue === 'recette' ? 'Ajouter une recette' : 'Ajouter une dépense'}
        </h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                {(['depense', 'recette'] as const).map(t => (
                  <button
                    ...
                    style={{
                      ...
                      ...(typeValue === t
                        ? t === 'depense'
                          ? { background: 'rgba(251,57,54,0.08)', borderColor: '#FB3936', color: '#D42F2D' }
                          : { background: 'rgba(5,150,105,0.08)', borderColor: '#059669', color: '#059669' }
                        : { background: 'white', borderColor: 'rgba(192,48,46,0.15)', color: '#7F7F7F' }
                      ),
                    }}
                  >
          ...
          {/* Justificatif */}
          <div>
            <Label>Lien justificatif <span style={{ color: '#7F7F7F', fontWeight: 400, textTransform: 'none', letterSpacing: 0, fontSize: '12px' }}>— optionnel</span></Label>
            <input
              type="url"
              placeholder="https://…"
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <label style={{ display: 'block', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#7F7F7F', marginBottom: '6px' }}>
      {children}
    </label>
  )
}
