...
const TYPE_CONFIG: Record<EventType, { bg: string; border: string; color: string; headerBg: string }> = {
  race:        { bg: 'rgba(208,89,24,0.08)',    border: 'rgba(208,89,24,0.2)',    color: '#D05918',  headerBg: 'rgba(208,89,24,0.05)'    },
  outing:      { bg: 'rgba(58,107,42,0.08)',    border: 'rgba(58,107,42,0.2)',    color: '#3A6B2A',  headerBg: 'rgba(58,107,42,0.05)'    },
  competition: { bg: 'rgba(245,158,11,0.10)',   border: 'rgba(245,158,11,0.25)', color: '#d97706',  headerBg: 'rgba(245,158,11,0.06)'   },
  other:       { bg: 'rgba(122,158,110,0.10)',  border: 'rgba(122,158,110,0.2)', color: '#7A9E6E',  headerBg: 'rgba(122,158,110,0.05)'  },
}
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: '0 2px 8px rgba(30,58,20,0.07)',
    ...
    >
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-1.5">
            ...
            {!event.is_public && (
              <span className="rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: 'rgba(30,58,20,0.06)', color: '#7A9E6E' }}>
                Privé
              </span>
            )}
            {isPast && (
              <span className="rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: 'rgba(30,58,20,0.05)', color: '#7A9E6E' }}>
                Terminé
              </span>
            )}
          </div>
          <h3 className="truncate font-bold leading-tight" style={{ color: '#1E3A14' }}>{event.title}</h3>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col gap-3 px-4 py-4">
        {event.description && (
          <p className="line-clamp-2 text-sm leading-relaxed" style={{ color: '#7A9E6E' }}>{event.description}</p>
        )}

        <div className="mt-auto space-y-1.5">
          {event.location && (
            <div className="flex items-center gap-2 text-xs" style={{ color: '#7A9E6E' }}>
      ...
      <div
        ...
        style={{ borderTop: '1px solid rgba(30,58,20,0.06)' }}
      >
