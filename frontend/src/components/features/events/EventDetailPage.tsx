            ...
            {user && (canManageEvents || user.id === event.created_by) && photos.length < 20 && (
              <>
                ...
                <button
                  ...
                  style={{ background: '#D05918', boxShadow: '0 2px 8px rgba(208,89,24,0.25)' }}
                >
                  {uploading ? (
                    'Envoi…'
                  ) : (
          ...
          <div className="p-6">
            {photosLoading ? (
              <div className="grid grid-cols-3 gap-3 sm:grid-cols-3">
                {[1, 2, 3].map(i => (
                  <div key={i} className="aspect-square rounded-xl animate-pulse" style={{ background: 'rgba(30,58,20,0.06)' }} />
                ))}
              </div>
            ) : photos.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-center">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'rgba(30,58,20,0.25)', marginBottom: '0.75rem' }}>
                  <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" />
                  <circle cx="12" cy="13" r="4" />
                </svg>
                <p className="text-sm font-semibold" style={{ color: '#1E3A14', marginBottom: '0.25rem' }}>Aucune photo pour l&apos;instant</p>
                {user && (canManageEvents || user.id === event.created_by) ? (
                  <p className="text-xs" style={{ color: '#7A9E6E' }}>Partagez les souvenirs de cette sortie — les photos sont pour toujours !</p>
                ) : (
                  <p className="text-xs" style={{ color: '#7A9E6E' }}>Les organisateurs peuvent ajouter des souvenirs de cette sortie.</p>
                )}
              </div>
            ) : (
              <>
